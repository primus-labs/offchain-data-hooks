...
async function main() {
    ...
    try {
        const kychooks = await KYCFactoryContract.mineDeploy2(POOL_MANAGER,
            EASPROXY_ADDRESS, EAS_ADDRESS,
            SCHEMA_KYC_BYTES, SCHEMA_COUNTRY_BYTES, 701);
        console.log('kychooks=', kychooks);
    } catch (ex) {
        try {
            const tx = await KYCFactoryContract.mineDeploy2.staticCallResult(POOL_MANAGER,
                EASPROXY_ADDRESS, EAS_ADDRESS,
                SCHEMA_KYC_BYTES, SCHEMA_COUNTRY_BYTES, 201);
            console.log("tx=", tx);
        } catch (error) {
